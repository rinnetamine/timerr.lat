    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-gray-800/40 backdrop-blur-sm rounded-lg border border-gray-700 p-6">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-gray-200">Kontaktziņojumi</h2>
                        ...
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="text-sm text-gray-400">
                                    ...
                                    @if($message->user)
                                        Lietotājs: <a href="{{ route('people.show', $message->user->id) }}" class="text-neon-accent hover:text-neon-accent/80 transition-colors duration-200">
                            ...
                            <div class="flex flex-col items-end space-y-2">
                                <span class="px-2 py-1 rounded-full text-xs {{ $message->status === 'unread' ? 'bg-red-900/40 text-red-300' : 'bg-green-900/40 text-green-300' }}">
                                    {{ $message->status === 'unread' ? 'Neizlasīts' : 'Izlasīts' }}
                                </span>
                                <div class="flex space-x-2">
                                    <form action="{{ route('admin.contact.delete', $message) }}" method="POST" class="inline">
                                        ...
                                        @method('DELETE')
                                        <button type="submit" 
                                                onclick="return confirm('Dzēst šo ziņojumu? Šo darbību nevar atsaukt.')"
