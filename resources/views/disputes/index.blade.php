<x-layout>
    ...
    <div class="max-w-7xl mx-auto">
        <div class="mb-6">
            <h2 class="text-2xl font-bold text-white mb-2">Strīdu un admin pārskatīšanas pārvaldība</h2>
            <p class="text-gray-300">Pārskatiet un atrisiniet strīdus starp lietotājiem, un apstrādājiet admin pārskatīšanu no noraidītiem darbiem</p>
        </div>

        @if($disputes->count() === 0)
            <div class="bg-gray-900/60 border border-gray-700 rounded-lg p-8 text-center">
                <div class="text-gray-400 text-lg mb-2">Nav aktīvu strīdu</div>
                <p class="text-gray-500">Visi iesniegumi šobrīd darbojas gludi!</p>
            </div>
        @else
            <div class="space-y-4">
                @foreach($disputes as $dispute)
                    <div class="bg-gray-900/60 border border-gray-700 rounded-lg p-6 hover-card">
                            ...
                            <div class="flex-1">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                                    ...
                                    <div>
                                        ...
                                        @else
                                            <span class="text-white ml-2">Manuāla strīda</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                            <div class="ml-4">
                                ...
                                ">
                                    @if($dispute->status === \App\Models\JobSubmission::STATUS_ADMIN_REVIEW)
                                        Admin pārskatīšana
                                    @else
                                        @if($dispute->dispute_status === \App\Models\JobSubmission::DISPUTE_REQUESTED)
                                            Strīds pieteikts
                                        @elseif($dispute->dispute_status === \App\Models\JobSubmission::DISPUTE_UNDER_REVIEW)
                                            Pārskatīšanā
                                        @elseif($dispute->dispute_status === \App\Models\JobSubmission::DISPUTE_RESOLVED)
                        ...
                        @else
                            <div class="mb-4">
                                <p class="text-gray-300 text-sm mb-2"><strong>Atrisinājums:</strong></p>
                                <p class="text-green-200 bg-green-900/20 p-3 rounded border border-green-700">
                                    {{ $dispute->dispute_resolution }}
                                </p>
                            </div>
                            <div class="text-sm text-gray-400">
                                Atrisināja {{ $dispute->disputeResolver->first_name }} {{ $dispute->disputeResolver->last_name }} 
                                @if($dispute->dispute_resolved_at)
                                    {{ $dispute->dispute_resolved_at->translatedFormat('j. M Y, H:i') }}
                                @endif
                            </div>
                        @endif
                    </div>
