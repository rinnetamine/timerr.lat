    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl p-8" aria-hidden="false">
            ...
            <div class="relative text-center space-y-4">
                ...
                <h2 class="text-5xl md:text-6xl font-extrabold text-white/95 leading-tight">Exchange Skills with Time</h2>
                <p class="text-lg md:text-xl text-gray-300 max-w-2xl mx-auto">Share your expertise, earn time credits, and get the services you need — built around people and trust.</p>
                ...
                <form action="/jobs" method="GET" class="mt-6 max-w-3xl mx-auto flex items-center gap-3 bg-gray-900/30 border border-gray-800 rounded-lg p-1 shadow-lg">
                    ...
                    <div class="flex items-center gap-2 flex-1 px-3">
                        ...
                        <input id="search" name="search" type="search" placeholder="Search services, skills or people" class="w-full bg-transparent px-2 py-3 rounded-md text-gray-100 placeholder-gray-400 focus:outline-none" />
                    </div>
                    ...
                    <button class="ml-1 mr-1 px-4 py-2 rounded-md bg-neon-accent text-black font-semibold">Search</button>
                </form>
            </div>
        </div>
        ...
        <!-- Popular Categories -->
        <div class="mt-8">
            <h3 class="text-2xl font-semibold text-white/90 mb-4">Popular Categories</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach(array_slice(($categories ?? []), 0, 8) as $key => $group)
        ...
        <div class="mt-8">
            <div class="flex items-center justify-between mb-4">
                <h3 class="text-2xl font-semibold text-white/90">Featured Services</h3>
                <a href="/jobs" class="text-neon-accent">View all</a>
            </div>
                ...
                @foreach(($featuredJobs ?? []) as $job)
                    <a href="/jobs/{{ $job->id }}" class="block p-4 bg-gray-800/40 rounded-lg border border-gray-700 hover:border-neon-accent hover-card">
                        ...
                        <div class="mt-3 flex items-center justify-between">
                            <span class="bg-neon-accent/20 text-neon-accent px-3 py-1 rounded-full text-xs font-semibold">{{ $job->time_credits }} credits</span>
                            <span class="text-gray-400 text-xs">{{ $job->category }}</span>
                        </div>
                    </a>
                @endforeach
            </div>
